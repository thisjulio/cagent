...
export function discoverSubagents(cwd: string, roots = ["agents"]): SubagentCatalog {
  ...
  for (const root of roots) {
    ...
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
      ...
      try {
        ...
        if (!byName.has(agent.name)) byName.set(agent.name, { ...agent, source: file });
      } catch { /* invalid definitions are unavailable */ }
    }
  }
