...
function readYaml(file: string): Record<string, unknown> {
  ...
  return (
    (yaml.load(fs.readFileSync(file, "utf8")) as Record<string, unknown>) ?? {}
  );
}
...
export function loadConfig(cwd: string): AppConfig {
  ...
  return {
    ...
    model:
      (local.model as string | undefined) ??
      (global.model as string | undefined) ??
      process.env.CAGENT_MODEL,
    variant:
      (local.variant as string | undefined) ??
      (global.variant as string | undefined) ??
      process.env.CAGENT_VARIANT,
    compact_threshold_tokens:
      (local.compact_threshold_tokens as number | undefined) ??
      (global.compact_threshold_tokens as number | undefined),
    compact_threshold_percent:
      (local.compact_threshold_percent as number | undefined) ??
      (global.compact_threshold_percent as number | undefined),
    compact_auto:
      (local.compact_auto as boolean | undefined) ??
      (global.compact_auto as boolean | undefined) ??
      true,
    compact_keep_tokens:
      (local.compact_keep_tokens as number | undefined) ??
      (global.compact_keep_tokens as number | undefined) ??
      32000,
    compact_prune_tool_tokens:
    ...
    observability: {
      enabled:
        (local.observability as AppConfig["observability"] | undefined)
          ?.enabled ??
        (global.observability as AppConfig["observability"] | undefined)
          ?.enabled ??
        false,
      file:
        (local.observability as AppConfig["observability"] | undefined)?.file ??
        (global.observability as AppConfig["observability"] | undefined)?.file,
    },
    skills: {
      enabled:
        (local.skills as { enabled?: boolean } | undefined)?.enabled ??
        (global.skills as { enabled?: boolean } | undefined)?.enabled ??
        true,
      roots:
        (local.skills as { roots?: string[] } | undefined)?.roots ??
