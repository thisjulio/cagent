...
export function parseSkill(text: string, directoryName: string): SkillMetadata {
  ...
  if (!data || typeof data !== "object") throw new Error("frontmatter must be a YAML object");
  ...
  if (name !== directoryName) throw new Error(`skill name must match directory: ${directoryName}`);
  ...
  return {
    ...
    disableModelInvocation: readBoolean(value, "disable-model-invocation", false),
  };
}
