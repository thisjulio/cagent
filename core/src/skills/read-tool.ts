...
export function createReadSkillTool(catalog: SkillCatalog): ToolDefinition {
  return defineTool("skill", "Load a skill's SKILL.md and inject its instructions into the conversation.", {
    ...
    if (skill.metadata.disableModelInvocation) return { output: `skill is user-invocable only: ${skill.metadata.name}`, isError: true };
    ...
    if (resource === "SKILL.md") {
      ...
      if (content !== undefined) return { output: formatSkillToolOutput(skill.metadata.name, skill.directory, content) };
    }
