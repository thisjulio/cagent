...
async function latestVersion(): Promise<string> {
  ...
  if (!response.ok) throw new Error(`could not query the latest version (${response.status})`);
  const tag = (await response.json() as { tag_name?: unknown }).tag_name;
  if (typeof tag !== "string" || !/^v\d+\.\d+\.\d+$/.test(tag)) throw new Error("Release has no valid semver tag");
  return tag.slice(1);
}
...
export async function upgrade(): Promise<void> {
  if (!IS_RELEASE) {
    console.log("upgrade is only available in the installed binary; use a release build to update");
    return;
  }
  ...
  if (actual !== expectedChecksum(checksums, asset)) throw new Error("new binary checksum does not match");
  ...
  try {
    ...
    throw new Error(`could not replace ${executable}: ${error instanceof Error ? error.message : String(error)}`);
  }
