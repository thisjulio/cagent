  ...
  return (
    <box paddingX={2} marginTop={1} marginBottom={1} width="100%" flexDirection="column" flexShrink={0}>
      <text>
        ...
        {item.timestamp ? <span attributes={TextAttributes.DIM}> {time(item.timestamp)}</span> : null}
      </text>
      <text>
        <span fg="#d97757">│  </span>
        <span attributes={TextAttributes.DIM}>{compacted ? compacted[1] : item.content}</span>
      </text>
    </box>
  );
}
