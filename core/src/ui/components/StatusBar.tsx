export function StatusBar({ model, variant, tokens, inputTokens, outputTokens, contextWindow, threshold }: { model: string; variant?: string; tokens?: number; inputTokens?: number; outputTokens?: number; contextWindow: number; threshold: number }) {
  const label = variant ? `${model} (${variant})` : model;
  const pct = tokens !== undefined && contextWindow ? Math.round((tokens / contextWindow) * 100) : 0;
  ...
  return (
    <box border={["top"]} borderColor="#666666" height={2} flexShrink={0} flexDirection="row" justifyContent="space-between">
      ...
      <text fg="#666666">{tokenInfo} [{meter}] {pct}% context · /help</text>
    </box>
  );
}
