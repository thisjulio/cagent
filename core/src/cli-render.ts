...
export function renderHumanHistory(items: ChatItem[], color = process.stdout.isTTY): string {
  let hasAgentLabel = false;
  return items.map((item) => {
    const rendered = renderItem(item, color, hasAgentLabel);
    if (item.kind === "user") hasAgentLabel = false;
    if (item.kind === "thinking" || item.kind === "assistant" || item.kind === "tool") hasAgentLabel = true;
    return rendered;
  }).filter(Boolean).join("\n");
}

function renderItem(item: ChatItem, color: boolean, hasAgentLabel: boolean): string {
  ...
  if (item.kind === "assistant") return `${hasAgentLabel ? "" : "cagent\n"}└─ ${item.content}`;
  if (item.kind === "tool") {
