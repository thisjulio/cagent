...
describe("formatHeaderTitle", () => {
  it("uses the available width instead of a fixed title limit", () => {
    expect(formatHeaderTitle("Improve Session Titles", 80)).toBe("cagent | Improve Session Titles");
    expect(formatHeaderTitle("Improve Session Titles", 30)).toBe("cagent | Improve Session…");
  });
