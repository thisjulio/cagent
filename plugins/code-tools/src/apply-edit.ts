...
export function applyRegions(lines: string[], regions: Region[], threshold: number): { lines: string[]; error?: ApplyError } {
  const result = [...lines];
  for (const r of regions) {
    const m = findBlock(result, r.search, threshold);
    if (m === "ambiguous") return { lines: result, error: { code: "E_AMBIGUOUS", search: r.search } };
    if (!m) return { lines: result, error: { code: "E_NO_MATCH", search: r.search } };
...
export function applyHunks(lines: string[], file: PatchFile, threshold: number): { lines: string[]; error?: ApplyError } {
  ...
  for (const h of file.hunks) {
    ...
    if (m === "ambiguous") return { lines: result, error: { code: "E_AMBIGUOUS", search: h.oldLines.join("\n") } };
    if (!m) return { lines: result, error: { code: "E_NO_MATCH", search: h.oldLines.join("\n") } };
