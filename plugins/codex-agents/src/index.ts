...
function parse(file: string): SubagentDefinition {
  ...
  if (typeof data?.name !== "string" || typeof data?.description !== "string") throw new Error(`Codex agent requires name and description: ${file}`);
...
const register: Plugin = (ctx) => {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    if (entry.isFile() && entry.name.endsWith(".md")) ctx.registerSubagent(parse(path.join(dir, entry.name)));
  }
};
